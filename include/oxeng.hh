...
class builder_context{
  ...
  std::string                        m_target_basename {} ;
  std::string                        m_target{} ;
  std::string                        m_data_directory{} ;
  std::string                        m_string_path{} ;
  std::string                        m_source_directory{} ;
  //std::vector<std::string>          m_sorted_strings {} ;
  string_map_t  m_string_map{} ;
  std::map<std::string, std::string> m_lang_map{} ;
  std::map<std::string, std::string> m_file_map{} ;
  bool                               m_debug {false};
  std::map<std::string, std::map<std::string, std::string> > m_all_tsl_map{} ;
...
public:
  ...
  void load_def();
  void set_target(const char *t_target_path_argv);
  void set_string_path(const char *t_string_path_argv);
  void set_configuration_directory(const char *t_configuration_directory);
  void set_data_directory(const char *t_data_directory);
  void set_source_directory(const char *t_source_directory);
  void parse_target();
...
void canonize_file_format( std::string& input,std::string& output);
